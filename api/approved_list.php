...
{ $id = null;
    if(isset($_GET['user'])) $id = "and requested_by =  {$_GET['user']}";
    ...
    try{
      ...
      INNER JOIN major  ON major.major_id = c.major_id 
        WHERE approved = 1  and r_status = 1 $id";
...
try{
    ...
    $sql = "SELECT (cur_stu = stu_num) as stu, (cur_ex = ex_num) as ex FROM ta_request
            WHERE request_id = 36";
    $statement = $db->prepare($sql);
    $statement->execute([':request_id'=> $decode['request_id']] );
    $result = $statement -> fetch();
       if($result['stu'] == '1' && $result['ex'] == '1' ){
         $sql ="UPDATE matchinig_course SET m_status = 0 WHERE m_course_id = :m_course_id";
         $statement = $db->prepare($sql);
         $result = $statement->execute([
